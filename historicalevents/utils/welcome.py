...
def send_group_greeting(message: types.ChatMemberUpdated):
    try:
        ...
        if message.chat.type != "private" and new_member.status in [
            ...
            if chat_id in [CHANNEL, CHANNEL_POST, GROUP_LOG]:
                logger.warning(
                    f"Ignoring storage of chat with ID {chat_id}, as it matches a configured ID."
                )
                return
        ...
        logger.error(f"Error sending group greeting: {e}")
...
def on_left_chat_member(message):
    try:
        if message.left_chat_member.id == bot.get_me().id:
            ...
            logger.success(f"The bot has been removed from the group {chat_name} - ({chat_id})")
    except Exception as e:
